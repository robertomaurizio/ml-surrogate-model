...
def preprocess_data(df):
  ...
  # Concatenate scaled features from train, val, and test sets
  x_combined_scaled = np.vstack((x_train_scaled, x_val_scaled, x_test_scaled))

  # Define feature names
  feature_names = x.columns.tolist()

  fig, axs = plt.subplots(2, 2, figsize=(5, 4)) # 4 features, so 2x2 grid
  axs = axs.flatten()

  for i, feature_name in enumerate(feature_names):
      axs[i].hist(x_combined_scaled[:, i], bins=30, alpha=0.7, color='lightcoral')
      axs[i].set_title(f'Distribution of Standardized {feature_name}')
      axs[i].set_xlabel(f'Standardized {feature_name}')
  ...
  # Standardize the output targets using StandardScaler
  ...
  # Concatenate scaled targets from train, val, and test sets
  y_combined_scaled = np.vstack((y_train_scaled, y_val_scaled, y_test_scaled))
  ...
  for i, target_name in enumerate(target_names):
      axs[i].hist(y_combined_scaled[:, i], bins=30, alpha=0.7, color='lightgreen')
      axs[i].set_title(f'Distribution of Standardized {target_name}')
      axs[i].set_xlabel(f'Standardized {target_name}')
      axs[i].set_ylabel('Frequency')
      axs[i].grid(True)
...
def load_TPE_results():
  ...
  best_hyperparameters_path = os.path.join(load_directory, 'TPE_2025-12-27.pkl')
...
def load_model_history_list():
  ...
  load_directory = '/content/drive/My Drive/Colab Notebooks'
  list_save_path = os.path.join(load_directory, 'NN_model_history_list_2025-12-27.keras')
