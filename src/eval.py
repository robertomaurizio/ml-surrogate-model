...
from src.eval import evaluate_final_model
...
def evaluate_multiple_seeds(final_model_list):
  ...
  for i in range(len(final_model_list)):
    a, b, c = evaluate_final_model(final_model_list[i])
    mse_list.append(a)
    r2_list.append(b)
  ...
  return mean_mse, std_mse, mean_r2, std_r2, mse_list, r2_list
...
def plot_linear_regression_predictions(y_test, y_predicted_test_linear, r2_test_linear, mse_test_linear):
  # Plot predictions vs actual values
  ...
  plt.tight_layout()
  plt.show()
  return (mse_test_linear, r2_test_linear, y_predicted_test_linear)
